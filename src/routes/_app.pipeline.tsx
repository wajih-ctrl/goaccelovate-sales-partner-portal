...
function PipelineKanban() {
  ...
  const availableLeads = useMemo(
    () => leads.filter(l => l.status !== "Duplicate Under Review" && l.status !== "Duplicate Rejected"),
    [leads]
  );
  ...
  const orderedItems = (stage: LeadStage) => {
    ...
    return [...items].sort((a, b) => {
      ...
      return (ai === -1 ? 1000 + items.indexOf(a) : ai) - (bi === -1 ? 1000 + items.indexOf(b) : bi);
    });
  };
  ...
  const applyOrder = ({ leadId, targetStage, beforeId }: PendingMove) => {
    setOrder(prev => {
      ...
      STAGES.forEach(stage => {
        next[stage] = (prev[stage] || availableLeads.filter(l => l.stage === stage).map(l => l.id)).filter(id => id !== leadId);
      });
  ...
  return (
    <>
      <PageHeader title="Pipeline Kanban" description="Drag cards between columns or reorder them within a stage. Closed Won requires a confirmed deal value." />
      <PageContainer>
              ...
              >
                <div className="mb-3 flex items-center justify-between">
                  <div>
                    ...
                    <div className="text-[11px] text-muted-foreground">{items.length} - {fmtCurrency(total)}</div>
                  </div>
                </div>
                    ...
                    return (
                      <div
                        ...
                        onDrop={e => {
                          e.preventDefault();
                          e.stopPropagation();
                          if (dragging) requestMove({ leadId: dragging.id, targetStage: stage, beforeId: l.id });
                        }}
                      ...
                      >
                        <div className="flex items-start justify-between gap-2">
                          <Link to="/leads/$id" params={{ id: l.id }} className="block font-medium hover:underline">{l.company}</Link>
                          <span className="text-[10px] uppercase tracking-wider text-muted-foreground">Drag</span>
                        </div>
                        <div className="mt-0.5 text-xs text-muted-foreground">{p?.name}</div>
                        <div className="mt-2 flex items-center justify-between">
                          <span className="text-xs font-semibold">{fmtCurrency(l.estimatedValue)}</span>
                          <StatusBadge status={l.status} />
                        </div>
                        <div className="mt-1 text-[10px] text-muted-foreground">{daysSince(l.lastActivity)}d ago</div>
                        <select
                          value={l.stage}
      ...
      >
        ...
        <p className="text-xs text-muted-foreground">A commission line will be calculated from the partner's current rate and held until client payment is logged.</p>
      </FormDialog>
    </>
  );
}
