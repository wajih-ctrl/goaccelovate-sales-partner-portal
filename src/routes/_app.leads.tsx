...
function LeadsPage() {
  ...
  const visible = useMemo(() => {
    let list =
      user?.role === "partner" ? leads.filter((l) => l.partnerId === user.partnerId) : leads;
    if (q)
      list = list.filter(
        (l) =>
          l.company.toLowerCase().includes(q.toLowerCase()) ||
          l.contactName.toLowerCase().includes(q.toLowerCase()) ||
  ...
  const exportCsv = () => {
    const rows = [
      ["ID", "Company", "Contact", "Country", "Stage", "Status", "Value", "Last activity"],
    ];
    visible.forEach((l) =>
      rows.push([
        l.id,
  ...
  return (
    <>
      <PageHeader
        ...
        description={
          user?.role === "partner"
            ? "Leads you've submitted."
            : "Every lead across the GTPP network."
        }
        actions={
          <>
