...
const STAGES: (LeadStage | "All")[] = ["All", "New Lead", "In Conversation", "Discovery Call", "Proposal Sent", "Negotiation", "Closed Won", "Closed Lost"];

function PipelineList() {
  const { user } = useAuth();
  const { leads, partners, updateLeadStage, closeLeadWon } = useStore();
  if (user?.role === "partner") return <Navigate to="/leads" />;
  ...
  const regions = useMemo(() => ["All", ...Array.from(new Set(leads.map(l => l.country))).sort()], [leads]);
  ...
  const list = useMemo(() => {
    ...
    if (dateFrom) l = l.filter(x => new Date(x.createdAt).getTime() >= new Date(dateFrom).getTime());
    if (dateTo) l = l.filter(x => new Date(x.createdAt).getTime() <= new Date(dateTo + "T23:59:59").getTime());
    l.sort((a, b) => sortDesc ? b.estimatedValue - a.estimatedValue : a.estimatedValue - b.estimatedValue);
    return l;
  ...
  return (
    <>
        ...
        <Card className="shadow-card">
          <div className="flex flex-wrap items-center gap-2 border-b p-3">
            <div className="relative min-w-[200px] flex-1">
              ...
              <input value={q} onChange={e => setQ(e.target.value)} placeholder="Search company…" className="h-9 w-full rounded-md border bg-background pl-9 pr-3 text-sm" />
            </div>
            ...
            <select value={region} onChange={e => setRegion(e.target.value)} className="h-9 rounded-md border bg-background px-3 text-sm">
              {regions.map(r => <option key={r}>{r === "All" ? "All regions" : r}</option>)}
            </select>
            <input type="number" value={minValue} onChange={e => setMinValue(e.target.value)} placeholder="Min value" className="h-9 w-28 rounded-md border bg-background px-3 text-sm" />
            <input type="number" value={maxValue} onChange={e => setMaxValue(e.target.value)} placeholder="Max value" className="h-9 w-28 rounded-md border bg-background px-3 text-sm" />
            <input type="date" value={dateFrom} onChange={e => setDateFrom(e.target.value)} className="h-9 rounded-md border bg-background px-3 text-sm" />
            <input type="date" value={dateTo} onChange={e => setDateTo(e.target.value)} className="h-9 rounded-md border bg-background px-3 text-sm" />
            <Button variant="ghost" size="sm" onClick={() => { setQ(""); setStage("All"); setPartner("All"); setRegion("All"); setMinValue(""); setMaxValue(""); setDateFrom(""); setDateTo(""); }}>Reset</Button>
          </div>
            ...
            <table className="w-full text-sm">
              <thead className="bg-accent/40 text-xs uppercase tracking-wider text-muted-foreground">
                <tr>
                  ...
                  <th className="px-4 py-3 text-right cursor-pointer" onClick={() => setSortDesc(!sortDesc)}>Value {sortDesc ? "↓" : "↑"}</th>
                ...
                {list.map(l => {
                  ...
                  return (
                    <tr key={l.id} className="border-t hover:bg-accent/20">
                      ...
                      <td className="px-4 py-3 text-right font-medium">{fmtCurrency(l.estimatedValue)}</td>
                      <td className="px-4 py-3 text-xs text-muted-foreground">{daysSince(l.lastActivity)}d</td>
                    </tr>
                  );
                })}
                {list.length === 0 && <tr><td colSpan={7} className="py-10 text-center text-muted-foreground">No leads.</td></tr>}
              </tbody>
            </table>
          </div>
        </Card>
      ...
      <FormDialog
        open={!!closeLeadId}
        onOpenChange={(open) => { if (!open) setCloseLeadId(null); }}
