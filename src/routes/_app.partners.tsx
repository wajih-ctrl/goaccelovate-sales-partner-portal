...
function PartnersPage() {
  ...
  if (user?.role === "partner") return <Navigate to="/profile" />;
      ...
      <PageContainer>
        <Card className="shadow-card">
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {filtered.map(p => {
                const activeLeads = leads.filter(l => l.partnerId === p.id && l.status === "Active").length;
                ...
                return (
                  <tr key={p.id} className="border-t hover:bg-accent/20">
                    ...
                    <td className="px-4 py-3">{p.city}, {p.country}</td>
              ...
              {filtered.length === 0 && (
                <tr><td colSpan={7} className="py-10 text-center text-sm text-muted-foreground">No partners match your search.</td></tr>
              )}
            </tbody>
          </table>
        </Card>
