...
type AnnouncementPriority = "General" | "Important" | "Urgent";
  ...
  return (
    <>
      <PageHeader
        title="Announcements"
        description={
          isAdmin ? "Broadcast updates to partners." : "Latest news and updates from GoAccelovate."
        }
        actions={
          isAdmin ? (
                ...
                <div className="flex items-start justify-between gap-3">
                  <div>
                    ...
                    <div className="mt-0.5 text-xs text-muted-foreground">
                      {new Date(a.date).toLocaleDateString()} · Target: {a.target}
                    </div>
                  </div>
                  {isAdmin ? (
                    <div className="text-xs text-muted-foreground">{a.readBy.length} read</div>
                  ) : (
                    unread && (
                      <Button
                        variant="outline"
                        size="sm"
                        onClick={() => markAnnouncementRead(a.id, user!.partnerId!)}
                      >
                        Mark read
                      </Button>
                    )
      ...
      <FormDialog
        ...
        canSubmit={
          !!form.title.trim() &&
          !!form.body.trim() &&
          (form.target !== "Selected partners" || !!form.selectedPartners.trim())
        }
        onSubmit={() => {
          const target =
            form.target === "Selected partners"
              ? `Selected partners: ${form.selectedPartners.trim()}`
              : form.target;
          publishAnnouncement(
      ...
      >
        ...
        {form.target === "Selected partners" && (
          <label className="text-xs">
            Selected partner names or IDs
            <input
              className="mt-1 h-10 w-full rounded-md border bg-background px-3 text-sm"
              value={form.selectedPartners}
              onChange={(e) => setForm({ ...form, selectedPartners: e.target.value })}
              placeholder="p1, Priya Shah"
            />
          </label>
        )}
      </FormDialog>
