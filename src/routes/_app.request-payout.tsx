...
function RequestPayout() {
  ...
  const eligible = commissions.filter(
    (c) => c.partnerId === partnerId && (c.state === "Unpaid" || c.state === "Approved"),
  );
  ...
  const submit = () => {
    ...
    requestPayout(partnerId, selected, message.trim(), user.name);
    toast.success(`Payout request for ${fmtCurrency(total)} submitted`);
    nav({ to: "/payouts" });
  };

  if (!partnerId) return <Navigate to="/access-denied" />;

  return (
    <>
      <PageHeader
        title="Request Payout"
        description="Select eligible commissions and submit a request."
      />
      <PageContainer>
          ...
          {eligible.length === 0 ? (
            <div className="rounded-md border border-dashed p-8 text-center text-sm text-muted-foreground">
              ...
              <div className="mt-2 text-xs">
                Commissions become eligible once a deal is Closed Won and not already requested or
                paid.
              </div>
            </div>
          ) : (
            <>
              <div className="flex items-center justify-between">
                <div className="text-sm text-muted-foreground">
                  {eligible.length} eligible commission(s)
                </div>
                      ...
                      <label className="flex flex-1 items-center gap-3 cursor-pointer">
                        ...
                        <div>
                          ...
                          <div className="text-xs text-muted-foreground">
                            {c.rate}% · Closed {new Date(c.closedDate).toLocaleDateString()}
                          </div>
                        </div>
                      </label>
              ...
              <div>
                ...
                <textarea
                  ...
                  placeholder="Any notes for the GoAccelovate team…"
                />
              </div>
              <div className="flex items-center justify-between border-t pt-4">
                <div>
                  <div className="text-xs uppercase text-muted-foreground">Total request</div>
