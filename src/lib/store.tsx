...
interface AppActions {
  ...
  addPartnerDocument: (
    partnerId: string,
    payload: { name: string; type: PartnerDocument["type"] },
    actor: string,
  ) => void;
...
export function StoreProvider({ children }: { children: ReactNode }) {
  ...
  const [onboarding, setOnboarding] =
    useState<Record<string, Record<string, boolean>>>(INITIAL_ONBOARDING);
  ...
  const addAttachment: AppActions["addAttachment"] = useCallback((leadId, name, actor) => {
    setAttachments((a) => ({
      ...a,
      [leadId]: [...(a[leadId] || []), { id: uid("f"), name, date: nowIso() }],
    }));
  ...
  const approveDuplicate: AppActions["approveDuplicate"] = useCallback((leadId, actor) => {
    ...
    pushActivity({
      ...
      type: "system",
      user: actor,
      text: "Duplicate review: Accepted into pipeline",
    });
  ...
  const waiveCommission: AppActions["waiveCommission"] = useCallback((id, actor, reason) => {
    setCommissions((prev) =>
      prev.map((c) => {
        if (c.id !== id) return c;
        pushAudit({
  ...
  const addPartnerDocument: AppActions["addPartnerDocument"] = useCallback(
    (partnerId, payload, actor) => {
      ...
      pushAudit({
        ...
        module: "Users",
        details: `${partnerId}: ${payload.name}`,
      });
