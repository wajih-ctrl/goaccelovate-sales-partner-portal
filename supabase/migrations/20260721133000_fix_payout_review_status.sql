-- Keep conditional payout status values typed as the payout_status enum.
...
begin
  if not public.is_super_admin() then
    raise exception 'Only Super Admin can approve or reject payouts';
  end if;
  select * into row_before
  from public.payout_requests where id=target_payout for update;
  if row_before.id is null then raise exception 'Payout request not found'; end if;
  if row_before.status<>'Pending' then
    raise exception 'Only pending payout requests can be reviewed';
  end if;
  ...
  values(
    ...
    case when approve_request
      then 'Your payout request for '||row_before.amount||' was approved and is awaiting external payment.'
